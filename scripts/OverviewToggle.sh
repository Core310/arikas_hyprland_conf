...
# 3) Fall back to Rofi window switcher
if command -v rofi >/dev/null 2>&1; then
  rofi -show window -theme "$HOME/.config/rofi/config.rasi" || rofi -show window
  exit 0
fi

# If we get here, neither worked
notify-send "Overview" "Neither Quickshell nor AGS nor Rofi is working" -u low 2>/dev/null || true
exit 1
